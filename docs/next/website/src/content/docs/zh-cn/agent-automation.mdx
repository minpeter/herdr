...
`--kind` 选择受支持的智能体及其标准可执行文件。支持的 kind 是 `pi`、`claude`、`codex`、`gemini`、`cursor`、`devin`、`agy`、`cline`、`omp`、`mastracode`、`opencode`、`copilot`、`kimi`、`kiro`、`droid`、`amp`、`grok`、`hermes`、`kilo`、`qodercli`、`qwen`、`maki`、`muse` 和 `omo`。`omo` 是 OmO Native 的公开 kind 和显示标识；底层 `senpi` 可执行文件及持久化清单 ID 为兼容性而保留。`--` 后的参数会原样传给该可执行文件。
